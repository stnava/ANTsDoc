...
\verb antsRegistration, \verb antsApplyTransforms, \verb Atropos ~for segmentation, \verb N4BiasFieldCorrection ~for
inhomogeneity correction,  \verb KellyKapowski ~for estimating
thickness, \verb ImageMath ~image processing utilities
and, finally, \verb sccan ~for sparse dimensionality reduction.  There
...
Perhaps the most important are \verb antsCorticalThickness.sh ~as a
wrapper of several sub-programs that support our
cortical thickness pipeline \cite{TustisonACT}, \verb antsMultivariateTemplateConstruction2.sh ~for template construction
and \verb antsRegistrationSyN.sh ~as a basic interface to a few commonly used registration approaches.
...
The \verb -d ~option denotes image dimensionality, \verb -f ~option denotes the ``fixed'' image and the \verb -m ~option
...
the moving image.  Output is determined by \verb -o ~where the output
...
The \verb antsRegistration ~program itself is the central program encapsulating
...
and transformations that are available.  \verb antsRegistration ~allows multiple
similarity and optimization criteria as options.  The program is
wrapped in \verb antsRegistrationSyN.sh ~for normalization with ``out of the box''
parameters and in \verb antsMultivariateTemplateConstruction2.sh ~for 
...
You can use the  \verb -r ~option in \verb antsRegistration ~to
...
The usage of \verb antsApplyTransformsToPoints ~is nearly identical.
